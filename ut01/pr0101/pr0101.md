...
Para ver su contenido, seleccionamos el archivo y luego en el botón de Abrir. Se nos abrirá otra pestaña y veremos el contenido que tiene dentro. El contenido es el siguiente:  
Creado desde la interfaz web
...
Para eliminar un bucket, primero hay que vaciarlo. Clicamos en él y luego en **Eliminar**. Nos saldrá un aviso de que no está vacío, si estamos seguros, clicamos en **Vaciar bucket** dentro del mismo mensaje. Después, tendremos que poner *eliminar de forma permanente* y luego clicar en **Vaciar**.

![vaciar bucket](imagenes/15.png)

Ahora, para eliminarlo repetimos el mismo proceso. No nos saldrá el mensaje de que tiene contenido, ponemos el nombre del bucket y ahora clicamos en **Eliminar bucket**.

![eliminar bucket](imagenes/16.png)

Repetimos lo mismo con los otros dos buckets.

Para parar el laboratorio, basta con clicar en el botón de `■ End Lab`.

Para apagar el contenedor, escribimos el siguiente comando en la terminal de Docker:
```powershell
docker compose down
```

### Conclusión comparativa
Considero que la consola web es ideal para el aprendizaje en explorar los servicios y revisar la infraestructura de manera visual. Por otro lado, la CLI agiliza la administración del día a día, ya que permite ejecutar tareas mediante los comandos siendo más rapido que haciendo clics y esperar a que cargen las ventanas.  
Boto3 se ve útil cuando se quiere programar automatizaciones complejas e integra los servicios de AWS.
